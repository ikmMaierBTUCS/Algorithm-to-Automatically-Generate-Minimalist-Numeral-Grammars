...
def list_scfunctions(language):
    ...
    for scf in set_of_scfunctions:
        example=scf.insert(scf.inputrange[0])
        printout=printout+scf.root+':\t x -> '+str([round(coeff) for coeff in scf.mapping[:-1]])+'*x+'+str(round(scf.mapping[-1]))+', \t e.g. '+example.numeral+' is '+str(example.number)+'\n'
    print(printout)

def number_of_items(language):
    lex=create_lexicon(language)
    set_of_scfunctions=[]
    irreducible_count=0
    for entry in lex:
        ...
        if '_' in parse.root:
            ...
            irreducible_count=irreducible_count+1
    print(language+': '+str(len(set_of_scfunctions))+'+'+str(irreducible_count)+'='+str(len(set_of_scfunctions)+irreducible_count))
    output=len(set_of_scfunctions)+irreducible_count
    return output
    #print(language+' has '+str(len(set_of_scfunctions))+' number functions and '+str(irreducible_count)+' irreducible numbers.')
    #print('The number functions are:')
    #printout=f""
...
def get_scfunctional_equations(lexicon):
    #create empty array of scfunctions
    set_of_scfunctions = []
    #parse all lexicon entries to find scfunctions
    for entry in lexicon:
        parse=proto_parse(entry.number,entry.word,lexicon,False,True)
        if len(parse[1])>0: # check if parser finds an input. if it does not, the function is trivial
            root=parse[0]
            fct_found=False
            for function in set_of_scfunctions:
                if root==function[0]:
                    fct_found=True
                    if function[-1]<=len(parse[1]):
                        function[-1]=function[-1]+1
                        if function[-1]>len(parse[1]):
                            print('linear regression possible for '+scfct)
                    break
            if not fct_found:
                set_of_scfunctions=set_of_scfunctions+[parse+[1]]
                print(set_of_scfunctions)
                for scf in set_of_scfunctions:
                    print(scf[1][0].number)
...
def list_scfunctions(language):
    ...
    irreducible_count=0
    for entry in lex:
        parse=advanced_parse(entry.number,entry.word,lex,True,True)
        if '_' in parse.root:
            function_known=False
            for pos in range(len(set_of_scfunctions)):
                if set_of_scfunctions[pos].root == parse.root:
...
def old_number_of_items(language):
    lex=create_lexicon(language)
    set_of_scfunctions=[]
    irreducible_count=0
    for entry in lex:
...
def list_items(language):
    lex=create_lexicon(language)
    set_of_scfunctions=[]
    irreducible_count=0
    print(language+' with new parser')
    for entry in lex:
        parse=advanced_parse(entry.number,entry.root,lex,True,True)
        if '_' in parse.root:
            function_known=False
            for pos in range(len(set_of_scfunctions)):
                if set_of_scfunctions[pos].root == parse.root:
                    ...
                    break
            if not function_known:
                print(parse.root + '\t\t\t\t --> '+entry.word+'('+str(entry.number)+')')
                set_of_scfunctions=set_of_scfunctions+[parse]
        else:
            print(parse.root+'('+str(entry.number)+')')
            irreducible_count=irreducible_count+1
    print(language+': '+str(len(set_of_scfunctions))+'+'+str(irreducible_count)+'='+str(len(set_of_scfunctions)+irreducible_count))
    print()
    output=len(set_of_scfunctions)+irreducible_count
    return output
def old_list_items(language):
    lex=create_lexicon(language)
    set_of_scfunctions=[]
    irreducible_count=0
    print(language+' with old parser')
    for entry in lex:
        parse=proto_parse(entry.number,entry.word,lex,False,False)
        if '_' in parse.root:
            function_known=False
